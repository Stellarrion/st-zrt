...
for model in $models; do
  ...
  case "$model" in
    relay4m|relay16m)
      cargo run --locked --quiet --release --manifest-path bench-c/Cargo.toml --example mem_probe -- "$model" "$iters" allocated-output
      cargo run --locked --quiet --release --manifest-path bench-c/Cargo.toml --example mem_probe -- "$model" "$iters" allocated-io
      cargo run --locked --quiet --release --manifest-path bench-c/Cargo.toml --example mem_probe -- "$model" "$iters" device-output
      ;;
  esac
  cargo run --locked --quiet --release --manifest-path bench/Cargo.toml --example mem_probe -- "$model" "$iters"
done
